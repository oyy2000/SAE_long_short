#!/bin/bash
#SBATCH -J 1_11_phase1_train_confirm
#SBATCH -N 1
...
python3 -u scripts/1_23_launch_policy_sft_matrix.py --config "${CONFIG}" --stage confirm \
  --gpu-ids "${GPU_CSV}" --runtime-root "/var/tmp/${USER}-phase1-policy-confirm/${SLURM_JOB_ID}"
