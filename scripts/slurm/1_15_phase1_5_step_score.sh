...
for shard in 0 1 2; do
  CUDA_VISIBLE_DEVICES="${GPU_IDS[$shard]}" python3 -u scripts/1_15_score_long_trace_steps.py \
    --config "${CONFIG}" --utility-config "${UTILITY_CONFIG}" --shard-index "${shard}" --shard-count 3 \
    --output-dir "results/phase1_5_credit_allocation_v1/formal/step_scores/shard_0${shard}_of_03" \
    >"logs/phase1_5_step_score_${shard}-${SLURM_JOB_ID}.log" 2>&1 &
  pids+=("$!")
done
