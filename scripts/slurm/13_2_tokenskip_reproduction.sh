...
export OMP_NUM_THREADS="${SLURM_CPUS_PER_TASK:-4}" OPENBLAS_NUM_THREADS=1
# NCSU administrator-designated scratch; never use the node system /var filesystem.
NCSU_JOB_USER="$(id -un)"
export TMPDIR="/share/jekml/${NCSU_JOB_USER}/tmp/${NCSU_JOB_USER}-phase13-tokenskip-${SLURM_JOB_ID}"
...
"$SFT_PYTHON" "$CODE_ROOT/scripts/13_6_run_tokenskip_reproduction.py" "$@" --config "$CONFIG"
