...
python3 -u scripts/1_25_launch_credit_training.py --stage confirm --gpu-ids "${GPU_CSV}" \
  --config "${PHASE1_5_CONFIG}" --utility-config "${PHASE1_CONFIG}" \
  --runtime-root "/var/tmp/${USER}-phase1-5-confirm/${SLURM_JOB_ID}"
