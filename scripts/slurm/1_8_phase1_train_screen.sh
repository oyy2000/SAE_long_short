#!/bin/bash
#SBATCH -J 1_8_phase1_train_screen
#SBATCH -N 1
...
python3 -u scripts/1_23_launch_policy_sft_matrix.py --config "${CONFIG}" --stage screen \
  --gpu-ids "${GPU_CSV}" --runtime-root "/var/tmp/${USER}-phase1-policy-screen/${SLURM_JOB_ID}"
