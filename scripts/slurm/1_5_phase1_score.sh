...
for shard in 0 1 2; do
  CUDA_VISIBLE_DEVICES="${GPU_IDS[$shard]}" python3 -u scripts/1_7_score_ctv_gradient_shard.py \
    --config "${CONFIG}" \
    --candidate-shard "results/phase1_teaching_utility_v1/formal/ctv_inputs/score_shards/shard_0${shard}_of_03.jsonl" \
    --output-dir "results/phase1_teaching_utility_v1/formal/ctv_scores/shard_0${shard}_of_03" \
    >"logs/phase1_score_shard_${shard}-${SLURM_JOB_ID}.log" 2>&1 &
  pids+=("$!")
done
