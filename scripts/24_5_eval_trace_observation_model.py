#!/usr/bin/env python3
"""Evaluate one Phase-0 base model or adapter on the locked math QA split."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache-dir", default=None, help="Optional Hugging Face model cache directory.")
    parser.add_argument(
        "--local-files-only",
        action="store_true",
        help="Forbid model downloads and resolve only from the local cache.",
    )
...
def _load_model(
    ...
    model = AutoModelForCausalLM.from_pretrained(model_name, **model_kwargs, **load_kwargs)
    if adapter_path:
        try:
